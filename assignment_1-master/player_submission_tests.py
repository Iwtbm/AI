...
def beatAI(yourAgent, yourAgent1):
    ...
    try:
        ...
        for idx in range(2):
            ...
            move = moves[0]
            game, _, _ = game.forecast_move(move)
        winner, move_history, termination = game.play_isolation(time_limit=6000, print_moves=False)
        # print("\n", winner, " has won. Reason: ", termination)
        return winner
        # Uncomment to see game
        # print game_as_text(winner, move_history, termination, output_b)
...
def algorithmTest(yourAgent, algorithm, algorithm_name):
    ...
    try:
        ...
        for depth, exp_score in expected_depth_scores:
            move, score,_ = algorithm(player, sample_board, time_left, depth=depth, my_turn=True)
            print(score)
            if exp_score != score:
